...
from models.modules.cond_DDPM import GaussianDiffusion
from models.modules.OpenAI_Unet import UNetModel as OpenAI_UNet

import torch
from utils.utils_eval import _test_step, _test_end, get_eval_dictionary
import numpy as np
from pytorch_lightning.core.lightning import LightningModule
import torch.optim as optim
from typing import Any, List
import torchio as tio
from utils.patch_sampling import BoxSampler
from utils.generate_noise import gen_noise
...
class DDPM_2D(LightningModule):
    def __init__(self,cfg,prefix=None):
        ...
        model = OpenAI_UNet(
                            image_size =  (256 / cfg.get('rescaleFactor',256) / cfg.get('rescaleFactor',1)),
                            in_channels = 1,
                            model_channels = cfg.get('unet_dim',64),
                            out_channels = 1,
                            num_res_blocks = cfg.get('num_res_blocks',3),
                            attention_resolutions = (int(256)/int(32),int(256)/int(16),int(256)/int(8)),
        ...
        model,
        image_size = (int(256 / cfg.get('rescaleFactor',1 )),int(256 / cfg.get('rescaleFactor', 1))), # only important when sampling
    ...
    def configure_optimizers(self):
        return optim.Adam(self.parameters(), lr=self.cfg.get('lr', 0.0001))
